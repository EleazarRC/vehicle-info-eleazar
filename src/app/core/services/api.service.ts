...
import {
  ...
  VehicleType,
  Model,
} from '../../state/models/vehicle.model';
...
export class ApiService {
  ...
  getBrands(): Observable<Brand[]> {
    return this.http
      .get<ApiResponse<Brand>>(`${this.baseUrl}/getallmakes?format=json`)
  ...
  getVehicleTypesForMake(make: string): Observable<VehicleType[]> {
    return this.http
      .get<ApiResponse<VehicleType>>(
        `${this.baseUrl}/GetVehicleTypesForMake/${make}?format=json`
      )
      .pipe(map((response) => response.Results));
  }
